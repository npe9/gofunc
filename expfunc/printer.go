...
// The generator package implements printing of AST nodes.
package main
...
func (p *generator) init(output io.Writer, cfg *Config) {
	p.output = output
	p.Config = *cfg
...
func (p *generator) internalError(msg ...interface{}) {
	if debug {
...
// nesting level.
//
func (p *generator) nlines(n, min int) int {
	if n < min {
		return min
	}
...
//
func (p *generator) write0(data []byte) {
...
//
func (p *generator) write(data []byte) {
	i0 := 0
	for i, b := range data {
		switch b {
		case '\n', '\f':
		...
		case tabwriter.Escape:
			...
			// ignore escape chars introduced by generator - they are
...
func (p *generator) writeNewlines(n int, useFF bool) {
	if n > 0 {
		n = p.nlines(n, 0)
		if useFF {
...
func (p *generator) writeTaggedItem(data []byte, tag HTMLTag) {
...
//
func (p *generator) writeItem(pos token.Position, data []byte, tag HTMLTag) {
	fileChanged := false
	if pos.IsValid() {
...
//
func (p *generator) writeCommentPrefix(pos, next token.Position, isFirst, isKeyword bool) {
	if !p.last.IsValid() {
...
func (p *generator) writeCommentLine(comment *ast.Comment, pos token.Position, line []byte) {
...
func (p *generator) writeComment(comment *ast.Comment) {
	text := comment.Text
...
//
func (p *generator) writeCommentSuffix(needsLinebreak bool) (droppedFF bool) {
	for i, ch := range p.buffer {
		switch ch {
		case blank, vtab:
...
// formfeed was dropped from the whitespace buffer.
//
func (p *generator) intersperseComments(next token.Position, tok token.Token) (droppedFF bool) {
...
func (p *generator) writeWhitespace(n int) {
	// write entries
	var data [1]byte
	for i := 0; i < n; i++ {
...
//
func (p *generator) print(args ...interface{}) {
	for _, f := range args {
...
//
func (p *generator) commentBefore(next token.Position) bool {
...
// comments.
//
func (p *generator) flush(next token.Position, tok token.Token) (droppedFF bool) {
...
func (cfg *Config) Fprint(output io.Writer, node interface{}) (int, os.Error) {
	...
	// setup generator and print node
	var p generator
	p.init(output, cfg)
	go func() {
		switch n := node.(type) {
		...
		case *ast.File:
			...
			p.file(n)
		default:
			p.errors <- fmt.Errorf("generator.Fprint: unsupported node type %T", n)
			runtime.Goexit()
		}
