...
// the print functionality implemented in generator.go.
...
//
func (p *generator) linebreak(line, min int, ws whiteSpace, newSection bool) (printedBreak bool) {
...
// intersperse.
func (p *generator) setComment(g *ast.CommentGroup) {
...
func (p *generator) identList(list []*ast.Ident, indent bool, multiLine *bool) {
...
func (p *generator) keySize(pair *ast.KeyValueExpr) int {
...
func (p *generator) exprList(prev token.Position, list []ast.Expr, depth int, mode exprListMode, multiLine *bool, next token.Position) {
	if len(list) == 0 {
		return
	}
...
func (p *generator) parameters(fields *ast.FieldList, multiLine *bool) {
...
func (p *generator) signature(params, result *ast.FieldList, multiLine *bool) {
...
func (p *generator) isOneLineFieldList(list []*ast.Field) bool {
	if len(list) != 1 {
...
func (p *generator) setLineComment(text string) {
	p.setComment(&ast.CommentGroup{[]*ast.Comment{&ast.Comment{noPos, []byte(text)}}})
}


func (p *generator) fieldList(fields *ast.FieldList, isIncomplete bool, ctxt exprContext) {
	p.nesting++
	defer func() {
		p.nesting--
	}()
...
func (p *generator) binaryExpr(x *ast.BinaryExpr, prec1, cutoff, depth int, multiLine *bool) {
...
func (p *generator) expr1(expr ast.Expr, prec1, depth int, ctxt exprContext, multiLine *bool) {
	p.print(expr.Pos())
...
func (p *generator) expr0(x ast.Expr, depth int, multiLine *bool) {
	p.expr1(x, token.LowestPrec, depth, 0, multiLine)
}


// Sets multiLine to true if the expression spans multiple lines.
func (p *generator) expr(x ast.Expr, multiLine *bool) {
	const depth = 1
	p.expr1(x, token.LowestPrec, depth, 0, multiLine)
}
...
func (p *generator) stmtList(list []ast.Stmt, _indent int, nextIsRBrace bool) {
...
func (p *generator) block(s *ast.BlockStmt, indent int) {
...
func (p *generator) controlClause(isForStmt bool, init ast.Stmt, expr ast.Expr, post ast.Stmt) {
	p.print(blank)
	needsBlank := false
...
func (p *generator) stmt(stmt ast.Stmt, nextIsRBrace bool, multiLine *bool) {
	p.print(stmt.Pos())
...
//
func (p *generator) spec(spec ast.Spec, n int, doIndent bool, multiLine *bool) {
	switch s := spec.(type) {
	case *ast.ImportSpec:
		p.setComment(s.Doc)
		if s.Name != nil {
...
func (p *generator) genDecl(d *ast.GenDecl, multiLine *bool) {
	p.setComment(d.Doc)
...
//
func (p *generator) nodeSize(n ast.Node, maxSize int) (size int) {
...
func (p *generator) isOneLineFunc(b *ast.BlockStmt, headerSize int) bool {
	pos1 := b.Pos()
	pos2 := b.Rbrace
...
func (p *generator) funcBody(b *ast.BlockStmt, headerSize int, isLit bool, multiLine *bool) {
	if b == nil {
		return
	}
...
// Sets multiLine to true if the declaration spans multiple lines.
func (p *generator) funcDecl(d *ast.FuncDecl, multiLine *bool) {
	p.setComment(d.Doc)
...
// Sets multiLine to true if the declaration spans multiple lines.
func (p *generator) decl(decl ast.Decl, multiLine *bool) {
	switch d := decl.(type) {
	case *ast.BadDecl:
...
func (p *generator) file(src *ast.File) {
